...
int UserManager::Start(){
	...
	if(ret==SUCCESS){
		...
		set_user_count(ui);
		set_cur_user_id(ui+1);
	}else{
		printf("[Error   ]UserManager Start fail\n");
		return ret;
	}
	ShowAll();
	printf("UserManager Start Over\n");
	return SUCCESS;
}
...
int UserManager::SaveUsers(){
	for(int i=0;i<user_count_;i++){
		...
		if(users[i].db_flag()==FLAG_INSERT){
			if(db_svr_->InsertUser(&users[i]) == SUCCESS){
				printf("Insert User %d %s\n",users[i].user_id(),users[i].user_name());
			}else{
				printf("Insert User fail %d %s\n",users[i].user_id(),users[i].user_name());
				return -1;
			}
			users[i].set_db_flag(FLAG_INIT);
		}
